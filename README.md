...
Android-приложение для чтения данных из машины через ELM327 (Bluetooth). Сделано под Cadillac CTS 2-го поколения (2.8 V6, АКПП 6L50). Стандартная часть работает с любой машиной на CAN; для Volkswagen — поиск блоков, их идентификация и ошибки.
...
- марка по VIN (первые 3 знака: GM, VAG, Toyota, Lada, Hyundai/Kia): GM-шаги ($22, $A9) — только для GM; для VW — свои (см. «Volkswagen»); для остальных — блоки на 7E0–7E7, их идентификация (UDS $22 F1xx / KWP $1A) и ошибки (UDS $19 / KWP $18). Ручной GM-скан доступен всегда;
...
| Как тестировать | Порядок записи сессии и особенности машины: GM, VW, Toyota, Lada, Hyundai (выбор по VIN или вручную) |
...
Тексты вкладки — [guide.xml](app/src/main/res/values/guide.xml): массив на раздел, первый элемент — заголовок. Короткая инструкция `guide_steps` общая, ниже — «Подробности»: общие разделы + раздел машины (`guide_gm`, `guide_vag`, `guide_toyota`, `guide_lada`, `guide_hyundai`). Перевод — как у справки, копией в `values-en/`.

«Где разъём OBD» — рисунок места водителя с подсвеченным разъёмом и адаптером, по картинке на машину: `app/src/main/res/drawable-nodpi/obd_*.png`. Их рисует [make_obd_art.py](art/obd/make_obd_art.py) (SVG-исходники рядом, рендер — headless Edge): `python art/obd/make_obd_art.py [cts polo rav4 vesta solaris]`. Текста внутри нет — подписи в `guide.xml`. Положения — по klavkarr.com и форумам владельцев, у Polo проверено на машине. Реальное фото можно положить вместо любого PNG с тем же именем.
